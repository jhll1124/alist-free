...
このリポジトリは、**商用化される前の、最後の真に自由だった AList のフォーク**です。  
オリジナルプロジェクトは**商用化計画**を発表し、その中には**ハードウェアテレメトリの収集やエンタープライズ版の機能制限**が含まれる可能性があり、**プライバシー、透明性、ガバナンス**に関してコミュニティから大きな懸念の声が上がりました。  

そのため、このフォークでは：  
- **デフォルトブランチを `main` から `free` に変更**し、保存された最後の自由版であることを示しています。  
- **このリポジトリは今後メンテナンスされず、コードとバイナリリリースの長期保存のみを目的としています。**
