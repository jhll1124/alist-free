...
本仓库为 **AList 商业化前最后自由版本 v3.45.0 的留存分叉**。  
原项目已宣布 **商业化计划**，并尝试引入 **硬件信息收集与企业功能付费化**，社区因此对隐私、遥测、治理等问题产生了强烈担忧。  

为此，本仓库：  
- **默认分支已从 `main` 改为 `free`**，用于标记并保留最后一个自由版本。  
- **本仓库不再进行维护，仅用于代码与二进制 release 的长期保存**。
